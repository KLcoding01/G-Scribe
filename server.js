...
// ✅ All STRICT Summary rules apply ONLY to PT/OT VISIT notes generated by POST /generate.
// ✅ Evaluation endpoints (pt_generate_summary, eval/*, extract) are unchanged (NO visit summary enforcement).
// ✅ Muscle-specific enforcement remains PT-visit-only (POST /generate + discipline === "PT").
//
// Visit-note flow (POST /generate):
// 1) Generate note with strict 3-section formatting.
// 2) Validate format + visit-summary rules (PT/OT visit only).
// 3) Repair pass if needed.
// 4) PT only: muscle-topic enforcement + repair (non-fatal if still fails).
...
// ✅ FIXED: ensure ALL prefixes end with a space (prevents subtle validation/grammar bugs)
...
function validateGenerated({
  ...
  if (enforceVisitSummaryRules) {
    ...
    if (mentionsTher && !mentionsCue) {
      return { ok: false, reason: "If Summary mentions ther-ex/ther-act, it must include VC/TC for skilled cueing." };
    }
    ...
    if (!hasFunctionalAnchor) {
      return { ok: false, reason: "Summary must include at least one functional anchor (ADLs/gait/transfers/balance/stairs/functional mobility)." };
    }
  }
...
app.post("/generate", async (req, res) => {
  try {
    ...
    // SECOND: PT visit muscle enforcement (PT visit only)
...
function buildFieldsMap(reqBody) {
  ...
  return Object.fromEntries(Object.entries(fields).map(([k, v]) => [k, normalizeSpaces(String(v ?? ""))]));
}
